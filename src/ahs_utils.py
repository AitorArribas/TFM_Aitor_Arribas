...
def show_drive_and_local_detuning_v2(drive: DrivingField, local_detuning: LocalDetuning):
    """Plot the driving field and local detuning
    ...
    nombres = {'amplitude [rad/s]': '$\Omega$ / rad/s', 
               'detuning [rad/s]': '$\Delta_G$ / rad/s', 
               'phase [rad]': ''}
    nombres_largos = {'amplitude [rad/s]': 'Término de Rabi',
                      'detuning [rad/s]': 'Detuning global', 
                      'phase [rad]': 'fase'}
    ...
    for ax, data_name in zip(axes, drive_data.keys()):
        if data_name == 'phase [rad]':
            ...
            ax.plot(drive_data[data_name].times(), drive_data[data_name].values(), '.-')
        ax.set_ylabel(nombres[data_name], fontsize=12)
        ax.set_title(nombres_largos[data_name], fontsize=12)
        ax.grid(ls=':')
        
    local_detuning_data = local_detuning.magnitude.time_series
    pattern = local_detuning.magnitude.pattern.series   
    axes[-1].plot(local_detuning_data.times(), local_detuning_data.values(), '.-', label="pattern: " + str(pattern))
    axes[-1].set_ylabel('$\Delta_L$ / rad/s', fontsize=12)
    axes[-1].set_xlabel('$t$ / s', fontsize=12)
    axes[-1].set_title('Detuning local', fontsize=12)
    axes[-1].legend()
    axes[-1].grid(ls=':')
    plt.tight_layout()
    plt.show()
